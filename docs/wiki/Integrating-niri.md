...
If both of these files are missing, niri will create `$XDG_CONFIG_HOME/niri/config.kdl` with the contents of [the default configuration file](https://github.com/Yasuno-5555/Jackrose/blob/main/resources/default-config.kdl), which are embedded into the niri binary at build time.
...
The compatibility [niri.service](https://github.com/Yasuno-5555/Jackrose/blob/main/resources/compat/niri.service) brings up `graphical-session.target` as well as `xdg-desktop-autostart.target`.
